...
export const buildFragmentTemplate = () => /* glsl */ `
    ...
    void main() {
        ...
        // runningColor is the per-layer-adjusted colour that survived the
        // boolean compositing — it's the colour to show "inside" the mask.
        float a = clamp(runningAlpha * uMaskAlpha, 0.0, 1.0);
        gl_FragColor = vec4(mix(srcRgb, runningColor, a), src.a);
    }
`
...
export const buildLayerAdjustFunction = (slotIndex, params = {}) => {
    ...
    return /* glsl */ `
        ...
        vec3 applyLayerAdjust_${slotIndex}(vec3 rgb) {
            // Early-out: identity adjustments. Saves ~10 ops/pixel for
            // the common "no-op adjust" case (a freshly created layer
            // before the user touches the sliders).
...
export const buildBooleanChain = (chainEntries) => {
    ...
    // adjusted colour; the alpha starts as the first layer's alpha.
    lines.push(`float a_0 = evalLayer(0);`)
    lines.push(`vec3 c_0 = applyLayerAdjust_0(srcRgb);`)
    ...
    for (let i = 1; i < chainEntries.length; i += 1) {
        ...
        const a = `a_${i}`
        const c = `c_${i}`
        lines.push(`float ${a} = evalLayer(${i});`)
        lines.push(`vec3 ${c} = applyLayerAdjust_${i}(srcRgb);`)
        switch (op) {
            case 'add':
                // Blend colour proportional to the new layer's alpha;
                // bump total alpha (clamped).
