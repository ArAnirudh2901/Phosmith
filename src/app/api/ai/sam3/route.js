...
/* ═══════════════════════════════════════════════════════════════════════════
 * SAM 3.1 click-to-select semantic masking (route path is the legacy "sam2" wire name)
 *
 * The Next.js route here is a thin proxy in front of the local Python
 * service (`services/masking/`), which runs Meta SAM 3.1 via
...
/** Optional SAM 3.1 box prompt: JSON [x0, y0, x1, y1] in ORIGINAL image px. */
 ...
 *
 * The Python service returns a raw SAM 3.1 mask at *prepared* resolution.
...
export async function POST(request) {
  try {
    ...
    // Dedicated bucket: SAM 3.1 prompts are interactive (the Mask tool's
    ...
    const limited = rateLimitResponse(await enforceRateLimit('ai-sam3', userId))
    if (limited) return limited

    if (!MASKING_SERVICE_URL) {
      return NextResponse.json(
        { error: 'MASKING_SERVICE_URL is not configured. Start the masking service (bun run masking:dev) and set MASKING_SERVICE_URL in .env.local.' },
        { status: 501 },
      )
    }
    ...
    console.info(
      '[ai-sam3] image:', prepared.width, 'x', prepared.height,
    ...
    try {
      ...
      console.warn('[ai-sam3] edge refinement failed, returning raw mask:', err?.message)
      finalMask = result.buffer
    }

    console.info(
      '[ai-sam3] ✓ mask:', finalMask.length, 'bytes',
